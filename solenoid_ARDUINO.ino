/*
  ...
  this case, the Arduino boards opens the solenoid when it receives the character
  'D', and closes the solenoid when it receives the character 'C'.
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // if it's a capital D (ASCII 72), open the solenoid:
    if (incomingByte == 'D') {
      digitalWrite(ledPin, HIGH);
      digitalWrite(4, HIGH);
    }
    // if it's an C (ASCII 76) close the solenoid:
    if (incomingByte == 'C') {
...
/* Processing code for this example
  ...
  void draw() {
    ...
    if (mouseX > boxX - boxSize && mouseX < boxX + boxSize &&
      ...
      stroke(255);
      fill(153);
      // send an 'D' to indicate mouse is over square:
      port.write('D');
    }
    else {
